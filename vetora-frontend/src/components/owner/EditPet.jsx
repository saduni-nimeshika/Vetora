...
const EditPet = () => {
  ...
  const fetchPet = async () => {
    try {
      setFetching(true);
      const res = await api.get(`/api/v1/owner/pets/${id}`);
      const pet = res.data;
      setFormData({
        name: pet.name || '',
        species: pet.species || '',
      ...
      if (pet.profileImage) {
        setImagePreview(pet.profileImage);
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await api.put(`/api/v1/owner/pets/${id}`, petData);
      ...
      if (error.response?.status === 400) {
        const errorMsg = error.response?.data?.error || error.response?.data?.message || 'Invalid data. Please check all fields.';
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-800">Edit Pet</h1>
      </div>
